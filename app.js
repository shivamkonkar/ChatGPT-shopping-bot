...
async function createNewDriver(name, email) {
    console.log(name)
...
app.post('/save-question-data', async (req, res) => {
    const receivedData = req.body;
    // Process receivedData
    console.log(req.body)
    const currentUser = req.user;
    const userId = currentUser._id;
    User.findByIdAndUpdate(userId, { $set: { keywords: receivedData } }, { new: true })
  .then(updatedDocument => {
    console.log('Keywords updated successfully:', updatedDocument);
  })
  .catch(error => {
    console.error(error);
  });
